...
| **05** | City3 (도시) | `city_23_high.glb` | `models[3]` | **3D 파일** — GLB .bin 로드 | `models[3].particleCount: 50000` |
...
- **풀 크기**: `max(모든 shape의 particleCount)` — 현재 50,000 (City3 기준)
...
{ id: 3, name: 'City3', particleCount: 50000, ... },    // 씬 05: 50k
```
...
│   ├── sphereDeform.ts      # 씬 01: Sphere deform 효과 타입/설정
│   ├── sphereMetaball.ts    # 씬 02: Metaball orbital 효과 타입/설정
...
최대 크기: 15,000 × 12 = ~176 KB (City3는 50,000 × 12 = ~586 KB)
...
models = [
  ...
  { id: 3, name: 'City3', modelPath: '/models/city_23_high.glb', scale: 1.0, particleCount: 50000, ... },
]
```
...
**CPU 최적화**: hold 시 activeCount만큼만 루프 (예: Sphere 15k, City3 50k). 비활성 파티클은 sizeMultiplier=0 감쇠만 처리.
